...
- **Piston speed is runtime-settable too, via `PISTON_SPEED` (mm/s, specific piston or `ALL`)** —
  ...
  full-step commutation code, not a STEP/DIR/microstep-capable driver, so only the full-step
  portion of the actuator's published load curve is reachable (roughly 55–120mm/s at ~6.5N down
  to ~1.3N at 640mA; well below that the curve climbs past 10N, up to ~13-18N near 1mm/s — running
  slower than the "suggested full-step zone" costs nothing but smoothness, which doesn't matter
  here since the EM tracker's closed loop already tolerates missed steps). Default is `60mm/s`,
  the low end of that zone, picked for margin over speed per the project's stated priority: a
  missed step is fine, stalling under load is a fail state. Step timing runs off `micros()`, not
  `millis()`, specifically to make speeds above ~10mm/s reachable at all.
