...
- **Piston speed is runtime-settable too, via `PISTON_SPEED` (mm/s, specific piston or `ALL`)** —
  also feeds into the next `HOME`'s duration, same as range. The pistons are Actuonix S20-38
  linear actuators; the driver hardware is a plain H-bridge toggled directly by the firmware's
  full-step commutation code, not a STEP/DIR/microstep-capable driver, so in principle only the
  full-step portion of the actuator's published load curve is reachable (roughly 55–120mm/s at
  ~6.5N down to ~1.3N at 640mA). **In practice, bench-tested on real hardware, the actuator
  doesn't move reliably at all from 15mm/s up** — a clean failure threshold, not gradual
  degradation, so the published load curve doesn't explain it on its own. Likely cause
  (unconfirmed, not yet fixed): every move starts stepping immediately at the full commanded rate
  with no acceleration ramp from standstill, and the datasheet explicitly warns that full step
  "will not move" at higher speeds without one. I2C blocking from the AB pressure reads was ruled
  out as the cause on this bench setup, since those sensors aren't wired and `readAbPressures()`
  already skips undetected sensors. Default is `10mm/s`, comfortably under the observed failure
  threshold. Step timing runs off `micros()`, not `millis()`, since `millis()` resolution alone
  caps achievable speed around 10mm/s regardless — i.e. today's default is about as fast as the
  old timing model could reach anyway; the `micros()` switch mainly gives headroom for whenever
  the >15mm/s failure is diagnosed (most likely a step-rate ramp) rather than a win today.
...
PISTON_SPEED <id|ALL> <mm_per_s>  # sets step rate; also changes next HOME's duration
...
# msg/PistonSpeedCommand.msg (new)
string id                      # p1 | p2 | p3 | d1 | d2 | d3 | ALL
float32 mm_per_s
...
- Topics in: `/firmware/piston_cmd` (`PistonCommand`), `/firmware/home_cmd` (`HomeCommand`),
  `/firmware/piston_range_cmd` (`PistonRangeCommand`), `/firmware/piston_speed_cmd`
  (`PistonSpeedCommand`), `/firmware/valve_cmd` (`ValveCommand`, open-loop), `/firmware/ab_pid_cmd`
  (`AbPidCommand`, firmware-side pressure hold), `/firmware/regulator_cmd` (`RegulatorCommand`,
  shared +/- regulators), `/firmware/regulator_range_cmd` (`RegulatorRangeCommand`).
